class LynxBrowserController {
    ...
    navigateCurrentTab(rawUrl) {
        ...
        if (!finalUrl.includes('.') && !finalUrl.startsWith('file://') && !finalUrl.includes('localhost')) {
            const searchProvider = localStorage.getItem('lynx_search') || 'duckduckgo';
            let searchBase = 'https://duckduckgo.com/?q=';
            
            if (searchProvider === 'google') searchBase = 'https://www.google.com/search?q=';
            else if (searchProvider === 'bing') searchBase = 'https://www.bing.com/search?q=';
            else if (searchProvider === 'brave') searchBase = 'https://search.brave.com/search?q=';
            
            finalUrl = searchBase + encodeURIComponent(finalUrl);
